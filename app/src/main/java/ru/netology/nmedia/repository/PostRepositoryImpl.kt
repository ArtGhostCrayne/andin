...
//    }
    ...
    override fun getAllAsync(callback: PostRepository.GetAllCallback<List<Post>>) {
        PostsApi.retrofitService.getAll().enqueue(object :
            Callback<List<Post>> {
    ...
    override fun removeById(id: Long, callback: PostRepository.GetCallback<Unit>) {
        ...
        PostsApi.retrofitService.removeById(id).enqueue(object :
            Callback<Unit> {
    ...
    override fun unlikeById(id: Long, callback: PostRepository.GetCallback<Post>) {
        PostsApi.retrofitService.unlikeById(id).enqueue(object :
            Callback<Post> {
            override fun onResponse(call: Call<Post>, response: Response<Post>) {
                if (!response.isSuccessful) {
                    callback.onError(RuntimeException(response.message()))
    ...
    override fun save(post: Post, callback: PostRepository.GetCallback<Post>) {
        PostsApi.retrofitService.save(post).enqueue(object :
            Callback<Post> {
            override fun onResponse(call: Call<Post>, response: Response<Post>) {
                if (!response.isSuccessful) {
                    callback.onError(RuntimeException(response.message()))
...
//
//
//       return client.newCall(request).execute().body?.string().let { gson.run { fromJson(it,Post::class.java ) } }
//    }
...
//    }
//
//    override fun unlikeById(id: Long, callback: PostRepository.GetLikeCallback) {
//        val request: Request = Request.Builder()
//            .delete()
//            .url("${BASE_URL}/api/posts/$id/likes")
//            .build()
...
////    }
//
//    override fun save(post: Post,callback: PostRepository.GetCallback) {
//        val request: Request = Request.Builder()
